	...
	const { shipData, totalPR, totalDR } = useMemo( () => {
		...
		const shipData = researchShips.map( ( item ) => {
			...
			const devPrints = Math.max( 0,
				Math.floor( devLevelsIndex[ 30 ][ item.type * 2 + 1 ]
					- devLevels[ item.type * 2 + 1 ] - ( ship.devStage || 0 ) / 10 ) );
			const fatePrints = !item.fate ? 0 : Math.max( 0,
				Math.floor( fateLevelsIndex[ 5 ][ 1 ] - fateLevels[ 1 ]
		...
		renderSubComponent: ( row ) => {
			...
			return (
				<Grid container spacing={2}>
					<Grid item xs={6}>
						Dev Level: {flexRender( cells.devLevel.column.columnDef.cell, cells.devLevel.getContext() )}
						Dev Stage: {flexRender( cells.devStage.column.columnDef.cell, cells.devStage.getContext() )}
					</Grid>
					{Boolean( row.original.fate ) && (
						<Grid item xs={6}>
							Fate Level: {flexRender( cells.fateLevel.column.columnDef.cell, cells.fateLevel.getContext() )}
							Fate Stage: {flexRender( cells.fateStage.column.columnDef.cell, cells.fateStage.getContext() )}
						</Grid>
					)}
				</Grid>
			);
