...
const initialState: State = {
	...
	dailyExpected: 1550,
	points: 0,
	farming: [{ id: nanoid(), points: 180, oil: 10 + 25 * 6 + 40 }],
};
...
const eventSlice = createSlice({
	...
	reducers: {
		...
		setShop(state, { payload }: PayloadAction<{ shop: Record<string, number>; total: number }>) {
		...
		setDaily(
			...
			{
				payload,
			}: PayloadAction<{ daily: { id: string; name: string; amount: number }[]; total: number }>,
		) {
		...
		setFarming(state, { payload }: PayloadAction<{ id: string; points: number; oil: number }[]>) {
			state.farming = payload;
		},
		modifyFarming(
			state,
			{ payload }: PayloadAction<{ id: string; points?: number; oil?: number }>,
		) {
